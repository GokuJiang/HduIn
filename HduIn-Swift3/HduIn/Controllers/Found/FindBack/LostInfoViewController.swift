...
class LostInfoViewController: BaseViewController ,ZHDropDownMenuDelegate{
    
    var collectionView: UICollectionView!
    var searchBar = UITextField()
    
    var provider = APIProvider<FindBackTarget>()
    var model:[FindBack.FindoutLost] = []
    var excatModel: [FindBack.FindoutLost] = []
    var searchModel: [FindBack.FindoutLost] = []
    ...
    override func viewDidLoad() {
        ...
        self.footer.setRefreshingTarget(self, refreshingAction: #selector(self.footerRefresh))
    ...
    func searchType(_ index: Int,_ model:[FindBack.FindoutLost]){
        ...
        for i in 0..<model.count{
            ...
            if model[i].type == index && model[i].status == 1{
                self.excatModel.append(model[i])
            }
        }
    ...
    func getData(){
        ...
        _ = provider.request(.FindoutLost(limit,"0"))
...
extension LostInfoViewController: UITextFieldDelegate{
    ...
    func textDidChange(_ textField: UITextField) {
        if let keyword = textField.text{
            if keyword == "" {
                ...
                dismissMenu()
                _ = provider.request(.searchLost(keyword))
                    .mapArray(FindBack.FindoutLost.self)
                    .subscribe({ (event) in
                        switch event{
...
extension LostInfoViewController: UICollectionViewDelegate{
    
}
//MARK: - UICollectionViewDataSource
extension LostInfoViewController: UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if self.excatModel[indexPath.row].name == "一卡通"{
            cell.numContentLabel.text = String(describing: self.excatModel[indexPath.row].studentNumber)
        }
        cell.timeContentLabel.text = self.excatModel[indexPath.row].foundTime//.split("T")
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        vc.detailView.timeLable.text = self.excatModel[indexPath.row].foundTime//.split("T")
...
extension LostInfoViewController: UICollectionViewDelegateFlowLayout{
...
extension LostInfoViewController: TabViewController {
    func setupTabInfo() {
        let title = "丢失".localized()
        //self.title = title
        
        let tabBarNoamalImage = #imageLiteral(resourceName: "lostgrey").withRenderingMode(.alwaysOriginal)
        let tabBarSelectedImage:UIImage = #imageLiteral(resourceName: "lostblue").withRenderingMode(UIImageRenderingMode.alwaysOriginal)
