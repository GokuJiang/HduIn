...
public extension Date {
    ...
    private static func components(fromDate date: Date) -> DateComponents {
        return Calendar.current.dateComponents(Date.componentFlags(), from: date)
    }
    ...
    init(fromString string: String, format:DateFormat, timeZone: ExTimeZone = .Local)
    {
        if string.isEmpty {
            self.init()
            return
        ...
        switch timeZone {
        case .Local:
            zone = TimeZone.ReferenceType.local
        case .UTC:
    ...
    /**
     ...
     - Returns A new date object.
     */
    func dateBySubtractingMonths(months: Int) -> Date
    {
        var dateComp = DateComponents()
    ...
    /**
     ...
     - Returns A new date object.
     */
    func dateBySubtractingWeeks(weeks: Int) -> Date
    {
        var dateComp = DateComponents()
    ...
    /**
     ...
     - Returns A new date object.
     */
    func dateBySubtractingSeconds(seconds: Int) -> Date
    {
        var dateComp = DateComponents()
...
//        let components = NSCalendar.currentCalendar().components(flags, fromDate: self)
    ...
    {
        return Date().dateByAddingDays(days: 1).dateAtStartOfDay()
    }
    ...
    {
        return Date().dateBySubtractingDays(days: 1).dateAtStartOfDay()
    }
...
//    func monthDays () -> Int { return NSCalendar.currentCalendar().rangeOfUnit(NSCalendarUnit.Day, inUnit: NSCalendarUnit.Month, forDate: self).length }
    ...
    func isWeekend() -> Bool {
        ...
        return (self.weekday() == range?.lowerBound || self.weekday() == range?.upperBound)
    }
    ...
    func toString() -> String {
        return self.toString(dateStyle: DateFormatter.Style.short, timeStyle: .short, doesRelativeDateFormatting: false)
    }
     ...
     */
    func toString(format: DateFormat, timeZone: ExTimeZone = .Local) -> String
    {
        var dateFormat: String
        let zone: TimeZone
        switch format {
        case .DotNet:
            let offset = TimeZone.ReferenceType.default.secondsFromGMT() / 3600
     ...
     */
    func toString(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, doesRelativeDateFormatting: Bool = false, timeZone: TimeZone = TimeZone.ReferenceType.local, locale: Locale = Locale.current) -> String
    {
    ...
    func relativeTimeToString() -> String
    {
        ...
        if seconds < 10 {
            ...
            let relativeTime = NSLocalizedString(describe(time: "%.f seconds"), comment: "Show the relative time from a date")
            return String(format: relativeTime, seconds)
        }
        
        if minutes < 60 {
            if minutes == 1 {
                return NSLocalizedString(describe(time: "1 minute"), comment: "Show the relative time from a date")
            } else {
                let relativeTime = NSLocalizedString(describe(time: "%.f minutes"), comment: "Show the relative time from a date")
        ...
        if hours < 24 {
            if hours == 1 {
                return NSLocalizedString(describe(time: "1 hour"), comment: "Show the relative time from a date")
            } else {
                let relativeTime = NSLocalizedString(describe(time: "%.f hours"), comment: "Show the relative time from a date")
        ...
        if days < 7 {
            if days == 1 {
                return NSLocalizedString(describe(time: "1 day"), comment: "Show the relative time from a date")
            } else {
                let relativeTime = NSLocalizedString(describe(time: "%.f days"), comment: "Show the relative time from a date")
     ...
     */
    private static func formatter(format:String = DefaultFormat, timeZone: TimeZone = TimeZone.ReferenceType.local, locale: Locale = Locale.current) -> DateFormatter {
    ...
    /**
     ...
     - Returns The date formatter.
     */
    private static func formatter(dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, doesRelativeDateFormatting: Bool, timeZone: TimeZone = TimeZone.ReferenceType.local, locale: Locale = Locale.current) -> DateFormatter {
        var formatters = Date.sharedDateFormatters
